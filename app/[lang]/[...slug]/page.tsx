...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = post?.frontmatter.title || slug[slug.length - 1].replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
  const description = post?.frontmatter.description || `Read ${title} in Sanskrit, English, and Hindi.`;
